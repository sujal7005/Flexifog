// server/routes/tvEntertainmentRoutes.js
...
import {
  ...
  getTVByResolution,
  getTVByScreenSize
} from '../controllers/tvEntertainmentController.js';
...
// Public routes
router.get('/', getTVProducts);
router.get('/:id', getTVProductById);
router.get('/brand/:brand', getTVByBrand);
router.get('/type/:type', getTVByType);
router.get('/resolution/:resolution', getTVByResolution);
router.get('/screen-size', getTVByScreenSize);

// Protected routes (admin only) - you'll need to add auth middleware
router.post('/', uploadProductMiddleware, createTVProduct);
router.put('/:id', uploadProductMiddleware, updateTVProduct);
router.delete('/:id', deleteTVProduct);
router.post('/:id/reviews', uploadReviewMiddleware, addTVReview);
