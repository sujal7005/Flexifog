...
const OfficePCs = () => {
  ...
  useEffect(() => {
    const fetchPCs = async (page = 1, limit = 10) => {
      try {
        ...
        console.log('Office PCs data:', data);
        
        // Get office PCs
        const allPCs = data.officePC || [];
        ...
        console.log(err);
        setError("Failed to fetch Office PCs");
        setLoading(false);
      }
    };
  ...
  const filters = React.useMemo(() => {
    ...
    pcs.forEach(pc => {
      if (pc.brand) brands.add(pc.brand);
      ...
      if (specs.cpu) {
        ...
        } else if (cpuStr.includes('xeon')) {
          processors.add('Intel Xeon');
        }
      }

      // Extract RAM
      if (specs.ram) {
        const ramMatch = String(specs.ram).match(/(\d+)\s*GB/i);
        if (ramMatch) {
      ...
      // Extract Storage
      if (specs.storage) {
        const storageMatch = String(specs.storage).match(/(\d+)\s*GB|\d+\s*TB/i);
        if (storageMatch) {
          storageOptions.add(storageMatch[0]);
        }
      } else if (specs.ssd) {
        const storageMatch = String(specs.ssd).match(/(\d+)\s*GB|\d+\s*TB/i);
        if (storageMatch) {
          storageOptions.add(storageMatch[0]);
        }
      } else if (specs.hdd) {
        const storageMatch = String(specs.hdd).match(/(\d+)\s*GB|\d+\s*TB/i);
        if (storageMatch) {
          storageOptions.add(storageMatch[0]);
        }
      }

      // Extract features specific to office PCs
      if (description.includes('business') || name.includes('business')) features.add('Business');
      if (description.includes('professional') || name.includes('professional')) features.add('Professional');
      if (description.includes('workstation') || name.includes('workstation')) features.add('Workstation');
      if (description.includes('compact') || name.includes('compact')) features.add('Compact');
      if (description.includes('quiet') || name.includes('quiet') || specs.silent) features.add('Quiet Operation');
      ...
      if (specs.vPro || description.includes('vpro')) features.add('Intel vPro');
      if (specs.tpm || description.includes('tpm')) features.add('TPM Security');
      if (specs.warranty) {
        if (String(specs.warranty).includes('3')) features.add('3+ Year Warranty');
      }
    });
    ...
    ? pcs
      .filter((pc) => {
        ...
        if (category !== "All") {
          ...
          if (categoryLower.includes('business') && !name.includes('business') && !description.includes('business')) {
            return false;
          }
          if (categoryLower.includes('professional') && !name.includes('professional') && !description.includes('professional')) {
            return false;
          }
          if (categoryLower.includes('workstation') && !name.includes('workstation') && !description.includes('workstation')) {
            return false;
          }
          if (categoryLower.includes('all-in-one') && !name.includes('all-in-one') && !description.includes('all-in-one')) {
            return false;
          }
          if (categoryLower.includes('compact') && !name.includes('compact') && !description.includes('compact')) {
            return false;
          }
        }
        ...
        if (searchTerm) {
          ...
          const name = safeToLowerCase(pc.name);
          const description = safeToLowerCase(pc.description);
          const brand = safeToLowerCase(pc.brand);
          if (!name.includes(searchLower) && !description.includes(searchLower) && !brand.includes(searchLower)) {
            return false;
          }
        }

        // Brand filter
        if (selectedBrand !== 'all' && pc.brand !== selectedBrand) {
          return false;
        }
        ...
        if (selectedProcessor !== 'all') {
          ...
          if (selectedProcessor === 'Intel Xeon' && !cpu.includes('xeon')) {
            return false;
          }
        }
        ...
        // RAM filter
        if (selectedRam !== 'all') {
          const ram = safeToLowerCase(pc.specs?.ram || '');
        ...
        if (selectedStorage !== 'all') {
          const storage = safeToLowerCase(pc.specs?.storage || pc.specs?.ssd || pc.specs?.hdd || '');
        ...
        // Price filter
        const price = pc.finalPrice || pc.price || 0;
        ...
        if (selectedFeatures.length > 0) {
          ...
          return selectedFeatures.every(feature => {
            switch(feature) {
              case 'Business':
                return name.includes('business') || description.includes('business');
              case 'Professional':
              ...
              case 'Compact':
                return name.includes('compact') || description.includes('compact');
              case 'Quiet Operation':
                return name.includes('quiet') || description.includes('quiet') || specs.silent;
              case 'WiFi':
                return specs.wifi || description.includes('wifi');
              case 'Bluetooth':
                return specs.bluetooth || description.includes('bluetooth');
              case 'Intel vPro':
                return specs.vPro || description.includes('vpro');
              case 'TPM Security':
                return specs.tpm || description.includes('tpm');
              case '3+ Year Warranty':
                return String(specs.warranty).includes('3');
              default:
                return true;
            }
          });
  ...
  const getCategoryIcon = (pc) => {
    ...
    if (name.includes('professional') || description.includes('professional')) {
      return { icon: <FaChartLine className="text-blue-400" />, color: 'from-blue-600 to-blue-800' };
    }
    if (name.includes('compact') || description.includes('compact')) {
      return { icon: <FaDesktop className="text-green-400" />, color: 'from-green-600 to-green-800' };
    }
    if (name.includes('all-in-one') || description.includes('all-in-one')) {
      return { icon: <FaDesktop className="text-purple-400" />, color: 'from-purple-600 to-purple-800' };
    }
    return { icon: <FaBriefcase className="text-indigo-400" />, color: 'from-indigo-600 to-indigo-800' };
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 pt-24 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-lg font-medium mt-4">Loading Office PCs...</p>
          <p className="text-gray-400 text-sm mt-2">Please wait while we find the perfect business PC for you</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 pt-24 pb-20">
      <Helmet>
        <title>Office PCs - Professional & Reliable Computers for Work | 7HubComputers</title>
        <meta name="description" content="Discover high-performance office PCs designed for productivity, reliability, and efficiency. Perfect for business, professional work, and everyday computing." />
        <meta name="keywords" content="office pc, business computer, professional desktop, work computer, productivity pc" />
      </Helmet>
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold mb-4 flex items-center">
            <FaBriefcase className="mr-4 text-indigo-400" />
            <span className="bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">
              Office PCs
            </span>
          </h1>
          <p className="text-lg text-gray-400 max-w-3xl">
            Professional-grade computers designed for productivity, reliability, and seamless multitasking.
          </p>
          <p className="text-sm text-gray-500 mt-2">
            {filteredPCs.length} Office PCs found
          </p>
        </motion.div>
          ...
          {compareList.length > 0 && (
            ...
            >
              <div className="flex items-center gap-4">
                <span className="font-bold">{compareList.length} Office PC(s) selected for comparison</span>
                <button
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-6">
              ...
              <div className="space-y-2">
                <label className="flex items-center space-x-2 cursor-pointer">
                  ...
                  />
                  <span className="text-gray-300">All Office PCs</span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="radio"
                  ...
                  <span className="text-gray-300 flex items-center">
                    <FaBuilding className="mr-2 text-blue-400" /> Business
                  </span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                  ...
                  />
                  <span className="text-gray-300 flex items-center">
                    <FaChartLine className="mr-2 text-purple-400" /> Professional
                  </span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                  ...
                  />
                  <span className="text-gray-300 flex items-center">
                    <FaServer className="mr-2 text-orange-400" /> Workstation
                  </span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                    ...
                    name="category"
                    value="All-in-One"
                    checked={category === "All-in-One"}
                    onChange={handleCategoryChange}
                    className="text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-gray-300 flex items-center">
                    <FaDesktop className="mr-2 text-green-400" /> All-in-One
                  </span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input
                    ...
                    name="category"
                    value="Compact"
                    checked={category === "Compact"}
                    onChange={handleCategoryChange}
                    className="text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-gray-300 flex items-center">
                    <FaDesktop className="mr-2 text-cyan-400" /> Compact
                  </span>
                </label>
              </div>
            </div>
          ...
          <div className="flex-1">
            {currentPCs.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="relative h-48 bg-gray-900 p-4">
                          ...
                          <div className="absolute top-3 right-3 flex flex-col gap-1 z-10">
                            ...
                            <span className={`bg-gradient-to-r ${categoryInfo.color} text-white text-xs font-bold px-2 py-1 rounded shadow-lg flex items-center`}>
                              {categoryInfo.icon} {pc.category || 'Office PC'}
                            </span>
                          </div>
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex flex-wrap gap-2 text-xs mb-2">
                            ...
                            {pc.specs?.ram && (
                              <span className="bg-gray-700 px-2 py-1 rounded flex items-center text-gray-300">
                                <FaMemory className="mr-1 text-indigo-400 text-xs" /> {pc.specs.ram}
                              </span>
                            )}
                            {pc.specs?.storage && (
                              <span className="bg-gray-700 px-2 py-1 rounded flex items-center text-gray-300">
                                <FaHdd className="mr-1 text-indigo-400 text-xs" /> {pc.specs.storage}
                              </span>
                            )}
                          </div>

                          {/* Business Features Tags */}
                          <div className="flex flex-wrap gap-1 mb-2">
                            ...
                            {pc.specs?.vPro && (
                              <span className="text-xs bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded">vPro</span>
                            )}
                            {pc.specs?.tpm && (
                              <span className="text-xs bg-orange-900/30 text-orange-400 px-2 py-0.5 rounded">TPM</span>
                            )}
                            {pc.specs?.warranty && (
                              <span className="text-xs bg-indigo-900/30 text-indigo-400 px-2 py-0.5 rounded">
                                {pc.specs.warranty} Warranty
                              </span>
                            )}
                          </div>
              ...
              >
                ...
                <h3 className="text-xl font-medium text-white mb-2">No Office PCs found</h3>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <h3 className="font-bold text-lg">Business PCs</h3>
              <p className="text-sm text-white/70 mt-2">Reliable for daily operations</p>
            </motion.div>
            ...
            >
              ...
              <h3 className="font-bold text-lg">Professional PCs</h3>
              <p className="text-sm text-white/70 mt-2">Power for demanding tasks</p>
            </motion.div>
            ...
            >
              ...
              <p className="text-sm text-white/70 mt-2">Maximum performance</p>
            </motion.div>
          </div>
        </motion.div>
        ...
        >
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-2xl font-bold mb-4">Why Choose Our Office PCs?</h3>
              <p className="text-white/80 mb-6">
                Built for productivity and reliability, our office PCs are designed to handle demanding business tasks with ease.
              </p>
              <ul className="space-y-3">
                <li className="flex items-center gap-2">
                  <FaMicrochip className="text-green-400" />
                  <span>High-performance processors for multitasking</span>
                </li>
                <li className="flex items-center gap-2">
                  <FaShieldAlt className="text-green-400" />
                  <span>Reliable components for 24/7 operation</span>
                </li>
                <li className="flex items-center gap-2">
                  <FaBolt className="text-green-400" />
                  <span>Energy-efficient designs to reduce costs</span>
                </li>
                <li className="flex items-center gap-2">
                  <FaHeadphones className="text-green-400" />
                  <span>24/7 business support included</span>
                </li>
              </ul>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm">
                <h4 className="font-bold mb-2">Business Ready</h4>
                <p className="text-sm text-white/70">Pre-loaded with essential software</p>
              </div>
              <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm">
                <h4 className="font-bold mb-2">Extended Warranty</h4>
                <p className="text-sm text-white/70">1-3 years comprehensive coverage</p>
              </div>
              <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm">
                <h4 className="font-bold mb-2">Security Features</h4>
                <p className="text-sm text-white/70">TPM, vPro, and more</p>
              </div>
              <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm">
                <h4 className="font-bold mb-2">Customizable</h4>
                <p className="text-sm text-white/70">Configure to your needs</p>
              </div>
            </div>
          </div>
        </motion.div>
                    ...
                    {compareList.map(productId => {
                      ...
                      return (
                        <div key={productId} className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                          ...
                          <p className="text-indigo-400 font-bold mb-2 text-center">{formatPrice(pc.finalPrice || pc.price)}</p>
                          <button
                            onClick={() => {
